...
using Dma2_Stream0 =
    DmaStream<DMA2_BASE, 0, 0,
              2>; // DMA 2, Stream 2, channel 0, high priority (ADC1)
using Dma2_Stream1 =
    DmaStream<DMA2_BASE, 1, 2,
              2>; // DMA 2, Stream 2, channel 2, high priority (ADC3)
using Dma2_Stream2 =
    DmaStream<DMA2_BASE, 2, 1,
              2>; // DMA 2, Stream 2, channel 1, high priority (ADC2)

StreamHandler Dma2_Stream0_hdl =
    Dma2_Stream0::init(10, DataSize::_16BIT, false);
StreamHandler Dma2_Stream1_hdl =
    Dma2_Stream1::init(10, DataSize::_16BIT, false);
StreamHandler Dma2_Stream2_hdl =
    Dma2_Stream2::init(10, DataSize::_16BIT, false);
...
/*
...
#ifdef STM32H743xx
    adc->CR = ADC_CR_ADVREGEN | ADC_CR_BOOST_1 | ADC_CR_BOOST_0;
    ...
    adc->CFGR |= ADC_CFGR_DISCEN | ADC_CFGR_EXTEN_0 // Trigger on rising edge
...
#else
    adc->SMPR2 = ADC_SMPR2_SMP2_2 | ADC_SMPR2_SMP1_2;
    adc->SQR1 = 0; // Convert one channel
    ...
    periph[instance].stream->setEndTransferCallback(
        std::bind(stopTransfer, instance));
}
...
static int stm32adc_start(const uint8_t instance, const void *config,
...
const struct audioDriver stm32_adc_audio_driver = { .start = stm32adc_start,
                                                    .data = stm32adc_data,
                                                    .sync = stm32adc_sync,
                                                    .stop = stm32adc_stop,
                                                    .terminate =
