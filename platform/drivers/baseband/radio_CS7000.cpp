...
static uint8_t txpwr_lo =
    0; // APC voltage for TX output power control, low power
static uint8_t txpwr_hi =
    0; // APC voltage for TX output power control, high power
...
static uint8_t interpParameter(uint32_t freq, uint32_t *calFreq,
                               uint8_t param[8])
{
    uint8_t i;
    for (i = 6; i > 0; i--) {
...
static struct rssiParams interpRssi(const uint32_t freq,
                                    const struct rssiParams cal[7])
{
...
void radio_enableRx()
{
    ...
    if ((config->opMode == OPMODE_FM) && (config->rxToneEn == true))
        stm32_adc_audio_driver.start(STM32_ADC_ADC3, (void *)ADC_CTCSS_CH,
                                     &ctcssCtx);
...
void radio_enableTx()
{
    ...
    switch (config->opMode) {
        case OPMODE_FM: {
            ...
            FmConfig cfg = (config->bandwidth == BW_12_5) ?
                               FmConfig::BW_12p5kHz :
                               FmConfig::BW_25kHz;
...
void radio_updateConfiguration()
{
    ...
    vtune_rx = interpParameter(config->rxFrequency, calData.rxCalFreq,
                               calData.rxSensitivity);

    // APC voltage for TX output power control
    txpwr_lo = interpParameter(config->txFrequency, calData.txCalFreq,
                               calData.txMiddlePwr);
    txpwr_hi = interpParameter(config->txFrequency, calData.txCalFreq,
                               calData.txHighPwr);

    // HR_C6000 modulation amplitude
    uint8_t qAmp = interpParameter(config->txFrequency, calData.txCalFreq,
                                   calData.txDigitalPathQ);
    uint8_t iAmp = interpParameter(config->txFrequency, calData.txCalFreq,
