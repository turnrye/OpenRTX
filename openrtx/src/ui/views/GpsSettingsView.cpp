...
void GpsSettingsView::writeValueText(uint8_t row)
{
    ...
    if (row == RowTimezone) {
        ...
        const bool on = (row == RowEnabled) ? st.gps_enabled : st.gpsSetTime;
...
void GpsSettingsView::adjust(int dir)
{
    ...
    switch (editRow_) {
        case RowEnabled:
            st.gps_enabled = !st.gps_enabled;
            lastEnabled_ = st.gps_enabled;
            break;
        case RowSetTime:
            st.gpsSetTime = !st.gpsSetTime;
            lastSetTime_ = st.gpsSetTime;
            break;
        default: { /* RowTimezone */
            int tz = st.utc_timezone + dir;
            if (tz < kTzMin)
                tz = kTzMin;
            if (tz > kTzMax)
                tz = kTzMax;
            st.utc_timezone = (int8_t)tz;
...
void GpsSettingsView::syncFromState(const state_t &s)
{
    ...
    if (!(editing_ && editRow_ == RowEnabled)
        && (st.gps_enabled != lastEnabled_)) {
        lastEnabled_ = st.gps_enabled;
        writeValueText(RowEnabled);
        changed = true;
    }
    if (!(editing_ && editRow_ == RowSetTime)
        && (st.gpsSetTime != lastSetTime_)) {
        lastSetTime_ = st.gpsSetTime;
        writeValueText(RowSetTime);
        changed = true;
    }
