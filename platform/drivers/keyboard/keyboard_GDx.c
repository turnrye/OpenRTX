...
keyboard_t kbd_getKeys()
{
    ...
    delayUs(1);
    if (gpio_readPin(KB_ROW0) == 0)
        keys |= KEY_1;
    if (gpio_readPin(KB_ROW1) == 0)
        keys |= KEY_2;
    if (gpio_readPin(KB_ROW2) == 0)
        keys |= KEY_3;
    if (gpio_readPin(KB_ROW3) == 0)
        keys |= KEY_ENTER;
#ifdef PLATFORM_DM1801
    if (gpio_readPin(KB_ROW4) == 0)
        keys |= KEY_F4;
#else
    if (gpio_readPin(KB_ROW4) == 0)
        keys |= KEY_RIGHT;
    ...
    if (gpio_readPin(FUNC_SW) == 0)
        keys |= KEY_F1;
    if (gpio_readPin(FUNC2_SW) == 0)
        keys |= KEY_F2;
    if (gpio_readPin(MONI_SW) == 0)
        keys |= KEY_MONI;
