...
static const uint8_t initSeq2[] = {
    0x04, 0x11, 0x80, 0x0C, 0x22, 0x01, 0x00, 0x00, 0x33, 0xEF, 0x00, 0xFF,
    0xFF, 0xFF, 0xF0, 0xF0, 0x10, 0x00, 0x00, 0x06, 0x3B, 0xF8, 0x0E, 0xFD,
    0x40, 0xFF, 0x00, 0x0B, 0x00, 0x00, 0x00, 0x06, 0x0B, 0x00, 0x17, 0x02,
    0xFF, 0xE0, 0x14, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};
static const uint8_t initSeq3[] = { 0x01, 0x10, 0x69, 0x69, 0x96, 0x96, 0x96,
                                    0x99, 0x99, 0x99, 0xA5, 0xA5, 0xAA, 0xAA,
                                    0xCC, 0xCC, 0x00, 0xF0, 0x01, 0xFF, 0x01,
                                    0x0F, 0x00, 0x00, 0x00, 0x00, 0x0D, 0x70,
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
static const uint8_t initSeq4[] = { 0x01, 0x30, 0x00, 0x00, 0x20, 0x3C,
                                    0xFF, 0xFF, 0x3F, 0x50, 0x07, 0x60,
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00

};
static const uint8_t initSeq5[] = { 0x01, 0x40, 0x00, 0x01, 0x01,
                                    0x02, 0x01, 0x1E, 0xF0 };
static const uint8_t initSeq6[] = { 0x01, 0x50, 0x00, 0x08, 0xEB, 0x78, 0x67 };
static const uint8_t initSeq7[] = { 0x01, 0x04, 0xD5, 0xD7,
                                    0xF7, 0x7F, 0xD7, 0x57 };
...
template <class M> void HR_Cx000<M>::init()
{
    ...
    writeReg(M::CONFIG, 0x10,
             0xF3); // FM mode, Tier II, TimeSlot, 3rd layer mode, aligned (?)
    writeReg(M::CONFIG, 0x40,
             0x43); // Enable RX synchronisation, normal mode (no test)
    ...
    writeReg(
        M::CONFIG, 0x01,
        0xF8); // Swap TX IQ, swap RX IQ, two point mode for TX, baseband IQ mode for RX
    ...
    writeReg(M::CONFIG, 0x06,
             0x20); // Vocoder output connected to universal interface (?)
    ...
    writeReg(M::CONFIG, 0x1E,
             0xF1); // Broadcast RX address, broadcast address mask
    writeReg(M::CONFIG, 0xE2, 0x00); // DAC off, mic preamp disabled
    writeReg(M::CONFIG, 0xE4,
             0x27); // Lineout gain, first and second stage mic gain
...
template <class M> void HR_Cx000<M>::dmrMode()
{
    writeReg(
        M::CONFIG, 0x10,
        0x4F); // DMR mode, Tier I, TimeSlot, 2nd layer mode, relay, aligned (?)
    writeReg(M::CONFIG, 0x81, 0x19); // Interrupt mask
    writeReg(
        M::CONFIG, 0x01,
        0xF0); // Swap TX IQ, swap RX IQ, two point mode for TX, IF mode for RX
    writeReg(M::CONFIG, 0xE4,
             0x27); // Lineout gain, first and second stage mic gain
    writeReg(M::CONFIG, 0xE5, 0x1A); // Undocumented register
    writeReg(M::CONFIG, 0x25, 0x0E); // Undocumented register
    ...
    writeReg(M::CONFIG, 0x41,
             0x40); // Start RX for upcoming time slot interrupt
    writeReg(M::CONFIG, 0x56, 0x00); // Undocumented register
    writeReg(M::CONFIG, 0x41,
             0x40); // Start RX for upcoming time slot interrupt
    writeReg(M::CONFIG, 0x5C, 0x09); // Undocumented register
    writeReg(M::CONFIG, 0x5F,
             0xC0); // Detect BS and MS frame sequences in 2 layer mode
...
{
    writeReg(M::CONFIG, 0x10,
             0x80); // FM mode, Tier II, TimeSlot, 3rd layer mode, aligned (?)
    writeReg(M::CONFIG, 0x01,
             0xB0); // Swap TX IQ, two point mode for TX, IF mode for RX
...
void HR_Cx000<M>::startAnalogTx(const TxAudioSource source, const FmConfig cfg)
{
    ...
    writeReg(
        M::CONFIG, 0x3E,
        0x08); // FM Modulation frequency deviation coefficient at the receiving end
