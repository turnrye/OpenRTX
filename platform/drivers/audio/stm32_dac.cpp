...
using Dma1_Stream5 =
    DmaStream<DMA1_BASE, 5, 7,
              3>; // DMA 1, Stream 5, channel 7, very high priority
using Dma1_Stream6 =
    DmaStream<DMA1_BASE, 6, 7,
              3>; // DMA 1, Stream 6, channel 7, very high priority
...
// Enable peripherals
    ...
    chState[instance].stream.setEndTransferCallback(
                          ...
                          struct streamCtx *ctx)
{
    ...
    miosix::markBufferBeforeDmaWrite(ctx->buffer,
                                     ctx->bufSize * sizeof(int16_t));
...
#pragma GCC diagnostic ignored "-Wpedantic"
const struct audioDriver stm32_dac_audio_driver = { .start = stm32dac_start,
