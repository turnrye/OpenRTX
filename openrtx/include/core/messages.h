 ...
 * dropped on the way out. Sequence numbers keep increasing across releases,
 * so a stale one never matches a later entry.
 *
 * Entries are kept in arrival order and exposed newest-first. When the entry
 * table or the body pool is full, the oldest entries are evicted to make room
 * for a new one.
 *
 ...
 * stores, deletes or runs the task; UI code must not cache them across ticks
 * and must pin an entry by its sequence number instead
 ...
 * the operating mode has a source.
 */
void messages_init(void);

/**
 * Shut down the message inbox, releasing its storage.
 */
 ...
 *         is longer than MSG_BODY_MAX_LEN - 1, -ENODEV if the inbox is
 *         inactive.
 */
