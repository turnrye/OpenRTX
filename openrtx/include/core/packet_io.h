 ...
 * much smaller queue (see pkt_tx_done_t) so a completion event can never
 ...
 * thread once a previously enqueued pkt_tx_request_t has finished (either
 * transmitted or failed).
 *
 * @param mode_id: opmode value identifying which protocol completed.
 * @param tag:     matches the originating pkt_tx_request_t's tag.
 * @param status:  a message_status_t value — MSG_STATUS_SENT or
 *                 MSG_STATUS_FAILED.
 */
typedef struct {
    uint8_t mode_id;
 ...
 * false result as "TX busy", matching the send() vtable contract's -EBUSY.
 *
 * @return true if enqueued, false if the single-slot queue is full.
 */
__attribute__((warn_unused_result)) bool
packet_io_enqueue_tx(const pkt_tx_request_t *req);
 ...
 */
__attribute__((warn_unused_result)) bool
packet_io_enqueue_rx(const pkt_rx_event_t *evt);
 ...
 */
__attribute__((warn_unused_result)) bool
packet_io_enqueue_tx_done(const pkt_tx_done_t *done);
 ...
 */
bool packet_io_dequeue_tx_done(pkt_tx_done_t *done);
