        ...
        if (newData) {
            ...
            if (status->lsfOk) {
                ...
                if ((streamType.fields.encType == M17_ENCRYPTION_NONE) &&
                    ...
                    std::string exCall1 =
                        decode_callsign(meta.extended_call_sign.call1);
                    std::string exCall2 =
                        decode_callsign(meta.extended_call_sign.call2);
                ...
                // matches with ours
                bool callMatch =
                    compareCallsigns(std::string(status->source_address), dst);
                ...
                if ((pthSts == PATH_CLOSED) && (canMatch == true) &&
                    (callMatch == true)) {
                    rxAudioPath =
                        audioPath_request(SOURCE_MCU, SINK_SPK, PRIO_RX);
