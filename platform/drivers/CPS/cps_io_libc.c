...
long _getBankDataOffset(uint16_t pos)
{
    ...
    if (pos == header.b_count) {
        ...
        if (header.b_count == 0)
            return ftell(cps_file) + header.ct_count * sizeof(contact_t) +
                   header.ch_count * sizeof(channel_t) + sizeof(uint32_t);
    ...
    fseek(cps_file,
          header.ct_count * sizeof(contact_t) +
              header.ch_count * sizeof(channel_t) + pos * sizeof(uint32_t),
          SEEK_CUR);
    uint32_t offset = 0;
    fread(&offset, sizeof(uint32_t), 1, cps_file);
    return ftell(cps_file) + (header.b_count - pos - 1) * sizeof(uint32_t) +
           offset;
}
...
int cps_readChannel(channel_t *channel, uint16_t pos)
{
    ...
    if (pos >= header.ch_count)
        return -1;
    fseek(cps_file,
          header.ct_count * sizeof(contact_t) + pos * sizeof(channel_t),
          SEEK_CUR);
...
int cps_readBankHeader(bankHdr_t *b_header, uint16_t pos)
{
    ...
    uint32_t offset = 0;
    fread(&offset, sizeof(uint32_t), 1, cps_file);
    fseek(cps_file, (header.b_count - pos - 1) * sizeof(uint32_t) + offset,
          SEEK_CUR);
...
int cps_readBankData(uint16_t bank_pos, uint16_t pos)
{
    ...
    fseek(cps_file,
          header.ct_count * sizeof(contact_t) +
              header.ch_count * sizeof(channel_t) + bank_pos * sizeof(uint32_t),
          SEEK_CUR);
    uint32_t offset = 0;
    fread(&offset, sizeof(uint32_t), 1, cps_file);
    fseek(cps_file, (header.b_count - bank_pos - 1) * sizeof(uint32_t) + offset,
          SEEK_CUR);
    bankHdr_t b_header = { 0 };
    fread(&b_header, sizeof(bankHdr_t), 1, cps_file);
...
int cps_insertChannel(channel_t channel, uint16_t pos)
{
    ...
    if (pos >= header.ch_count + 1)
        return -1;
    long ch_pos = ftell(cps_file) + header.ct_count * sizeof(contact_t) +
