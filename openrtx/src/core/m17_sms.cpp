...
#ifndef CONFIG_M17_SMS_MAX_MESSAGES
#define CONFIG_M17_SMS_MAX_MESSAGES 32
#endif
#ifndef CONFIG_M17_SMS_POOL_BYTES
#define CONFIG_M17_SMS_POOL_BYTES 3200
#endif

static_assert(CONFIG_M17_SMS_MAX_MESSAGES > 0,
              "CONFIG_M17_SMS_MAX_MESSAGES must be > 0");
static_assert(CONFIG_M17_SMS_POOL_BYTES >= PKT_BODY_MAX_LEN,
              "CONFIG_M17_SMS_POOL_BYTES must be >= PKT_BODY_MAX_LEN");
static_assert(CONFIG_M17_SMS_POOL_BYTES <= 65535,
              "CONFIG_M17_SMS_POOL_BYTES exceeds uint16_t range; "
...
static SmsEntry entries[CONFIG_M17_SMS_MAX_MESSAGES];
 ...
 */
static char body_pool[CONFIG_M17_SMS_POOL_BYTES];
 ...
 *
 * Advances pool_head by len+1 (wrapping at CONFIG_M17_SMS_POOL_BYTES).  Any active
...
static uint16_t pool_alloc(uint16_t len)
{
    ...
    if ((uint32_t)pool_head + need > CONFIG_M17_SMS_POOL_BYTES)
        pool_head = 0;
     ...
     * non-wrapping sub-ranges of [0, CONFIG_M17_SMS_POOL_BYTES) because pool_alloc()
    ...
    for (size_t i = 0; i < CONFIG_M17_SMS_MAX_MESSAGES; i++) {
        if (!entries[i].active)
            continue;
...
static size_t alloc_slot(void)
{
    ...
    size_t oldest = 0;
    for (size_t i = 1; i < CONFIG_M17_SMS_MAX_MESSAGES; i++) {
