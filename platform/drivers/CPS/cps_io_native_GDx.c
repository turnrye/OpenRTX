...
static const uint32_t channelBaseAddrEEPROM =
    0x03780; /**< Base address of channel data         */
static const uint32_t channelBaseAddrFlash =
    0x7b1c0; /**< Base address of channel data         */
static const uint32_t zoneBaseAddr =
    0x8010; /**< Base address of zones                */
static const uint32_t contactBaseAddr =
    0x87620; /**< Base address of contacts             */
static const uint32_t maxNumChannels =
    1024; /**< Maximum number of channels in memory */
static const uint32_t maxNumZones =
    68; /**< Maximum number of zones in memory    */
static const uint32_t maxNumContacts =
    1024; /**< Maximum number of contacts in memory */
...
int cps_readChannel(channel_t *channel, uint16_t pos)
{
    ...
    // ### Read desired channel from the correct bank ###
        ...
        if (pos < 128) {
            uint32_t bankAddr =
                channelBaseAddrEEPROM + bank_num * sizeof(gdxChannelBank_t);
            AT24Cx_readData(bankAddr + channelOffset, ((uint8_t *)&chData),
                            sizeof(gdxChannel_t));
        }
        // Remaining 7 channel banks (896 channels) are saved in SPI Flash
        else {
            uint32_t bankAddr =
                channelBaseAddrFlash + bank_num * sizeof(gdxChannelBank_t);
            nvm_devRead(&eflash, bankAddr + channelOffset, ((uint8_t *)&chData),
                        sizeof(gdxChannel_t));
        }
    }
...
int cps_readBankData(uint16_t bank_pos, uint16_t ch_pos)
{
    ...
    gdxZone_t zoneData;
    uint32_t zoneAddr =
        zoneBaseAddr + sizeof(bitmap) + bank_pos * sizeof(gdxZone_t);
    AT24Cx_readData(zoneAddr, ((uint8_t *)&zoneData), sizeof(gdxZone_t));
...
int cps_readContact(contact_t *contact, uint16_t pos)
{
    ...
    nvm_devRead(&eflash, contactAddr, ((uint8_t *)&contactData),
                sizeof(gdxContact_t));
