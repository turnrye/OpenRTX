...
3. Generates TTS audio using Piper
...
class VoicePromptGenerator:
    ...
    def generate_tts(self, text: str, output_wav: Path, voice: PiperVoice):
        ...
        # Use zero noise for deterministic, reproducible output
    ...
    def build_vpc(self, c2_files: List[Path], output_vpc: Path):
        ...
        - ToC (350 * 4 bytes): uint32 offsets to each codec2 data
    ...
    @staticmethod
    def check_vpc(reference: Path, generated: Path,
                  threshold: float = 0.01) -> bool:
        """
        Compare two VPC files, allowing minor audio drift from
        cross-platform floating-point differences in ONNX inference.

        Args:
            reference:  The committed VPC file.
            generated:  The freshly generated VPC file.
            threshold:  Maximum fraction of differing *bits* in the
                        audio body (0.0–1.0).  Default 0.01 (1 %).

        Returns:
            True if the files are equivalent within the threshold.
        """
        HEADER_SIZE = 8          # magic (4) + version (4)
        TOC_SIZE = VoicePromptGenerator.MAX_PROMPTS * 4  # uint32 offsets
        META_SIZE = HEADER_SIZE + TOC_SIZE

        ref_data = reference.read_bytes()
        gen_data = generated.read_bytes()

        # --- header comparison ---
        ref_header = ref_data[:HEADER_SIZE]
        gen_header = gen_data[:HEADER_SIZE]
        if ref_header != gen_header:
            print("FAIL: VPC header differs")
            return False

        # --- ToC comparison ---
        ref_toc = ref_data[HEADER_SIZE:META_SIZE]
        gen_toc = gen_data[HEADER_SIZE:META_SIZE]
        if ref_toc != gen_toc:
            print("FAIL: VPC table of contents differs")
            return False

        # --- audio body comparison (bit-level threshold) ---
        ref_audio = ref_data[META_SIZE:]
        gen_audio = gen_data[META_SIZE:]

        if len(ref_audio) != len(gen_audio):
            print(f"FAIL: audio length differs "
                  f"({len(ref_audio)} vs {len(gen_audio)} bytes)")
            return False
        ...
        total_bits = len(ref_audio) * 8
        diff_bits = sum(
            bin(a ^ b).count('1')
            for a, b in zip(ref_audio, gen_audio)
        )
    ...
    def generate_all(self, output_vpc: Path, temp_dir: Path = None,
                    ...
                    data_dir: Path = None,
                    gain_db: float = 0.0, tempo: float = 1.5,
...
Examples:
  ...
  %(prog)s --check --voice en_GB-alan-medium --tempo 1.5
        """
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.5,
        help='Audio tempo multiplier (default: 1.5)'
    )
    ...
    if args.check:
        ...
        try:
            ...
            ok = VoicePromptGenerator.check_vpc(
