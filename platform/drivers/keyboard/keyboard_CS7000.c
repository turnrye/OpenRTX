...
{
    static const uint8_t knobTable[] = { 0x09, 0x0A, 0x04, 0x03, 0x08, 0x07,
                                         0x05, 0x06, 0x0C, 0x0B, 0x01, 0x02,
                                         0x0D, 0x0E, 0x10, 0x0F };
...
keyboard_t kbd_getKeys()
{
    ...
    if (gpio_readPin(SIDE_KEY1) == 0)
        keys |= KEY_MONI;
    if (gpio_readPin(SIDE_KEY2) == 0)
        keys |= KEY_F1;
    if (gpio_readPin(SIDE_KEY3) == 0)
        keys |= KEY_F2;
    if (gpio_readPin(ALARM_KEY) == 0)
        keys |= KEY_F3;
