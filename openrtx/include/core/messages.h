 ...
 * UI-thread vtable callback that marks the snapshot dirty — tick(),
 ...
 * across any of them.
 *
 * CONFIG_MESSAGES: when a target does not define this flag, every facade
 * function below compiles to a cheap static-inline no-op/stub so callers
 * (state.c, threads.c, UI) do not need their own #ifdef guards.
 */
 ...
 *
 * Dispatches through the source vtable's supported_actions() callback.
 *
...
/* ---------------------------------------------------------------------- */
/* No-op stubs so callers never need their own #ifdef CONFIG_MESSAGES.     */
/* ---------------------------------------------------------------------- */

static inline uint32_t messages_alloc_sequence(void)
{
    return 0;
}

static inline size_t messages_find_by_sequence(uint32_t seq)
{
    (void)seq;
    return SIZE_MAX;
}

static inline uint32_t messages_supported_actions(size_t idx)
{
    (void)idx;
    return 0;
}

static inline void messages_init(void)
{
}

static inline void messages_terminate(void)
{
}

static inline size_t messages_tick(void)
{
    return 0;
}

static inline size_t messages_count(void)
{
    return 0;
}

static inline size_t messages_count_unread(void)
{
    return 0;
}

static inline message_header_t *messages_get(size_t idx)
{
    (void)idx;
    return NULL;
}

static inline int messages_invoke_action(size_t idx, message_action_t action)
{
    (void)idx;
    (void)action;
    return -ENOSYS;
...
static inline bool messages_can_compose(uint8_t mode)
{
    (void)mode;
    return false;
}

static inline int messages_start_compose(uint8_t mode)
{
    (void)mode;
    return -ENOSYS;
}

static inline uint8_t messages_source_mode(size_t idx)
{
    (void)idx;
    return 0;
}

static inline int messages_send(uint8_t mode, const char *body, size_t body_len,
                                const char *recipient)
{
    (void)mode;
    (void)body;
