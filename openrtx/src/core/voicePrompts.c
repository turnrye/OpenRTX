...
static vpSequence_t vpCurrentSequence = { .pos = 0,
                                          .length = 0,
                                          .c2DataStart = 0,
                                          .c2DataIndex = 0,
                                          .c2DataLength = 0 };
...
static void fetchCodec2Data(uint8_t *data, const size_t offset)
{
    ...
    size_t start =
        sizeof(vpHeader_t) + sizeof(tableOfContents) + CODEC2_HEADER_SIZE;

    fseek(vpFile, start + offset, SEEK_SET);
    fread(data, 8, 1, vpFile);
#else
    uint8_t *dataPtr = vpData + sizeof(vpHeader_t) + sizeof(tableOfContents) +
                       CODEC2_HEADER_SIZE;
                                             ...
                                             voicePrompt_t *vp)
{
    *vp = PROMPT_SILENCE;
    ...
    if (symbolPtr == NULL) { // we don't have a prompt for this character.
    ...
    while (vpCurrentSequence.pos < vpCurrentSequence.length) {
        ...
        if (vpCurrentSequence.c2DataLength == 0) {
            ...
            vpCurrentSequence.c2DataLength =
                ((tableOfContents[promptNumber + 1] -
        ...
        vpCurrentSequence.c2DataLength =
            0; // flag that we need to get more data.
        vpCurrentSequence.c2DataIndex = 0;
    }
