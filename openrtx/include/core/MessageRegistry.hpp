 ...
 * `MessageRegistry::init()`.  `vtable` must be non-NULL and its `count`/
...
public:
     ...
     *                 valid for the lifetime of the registry (i.e. until
     *                 terminate() is called).
...
inline void MessageRegistry::init(const SourceEntry *srcs, size_t count)
{
    sources = srcs;
    sourcesLen = count;
    snapshotLens[0] = 0;
...
inline void MessageRegistry::terminate()
{
    sources = nullptr;
    sourcesLen = 0;
    snapshotLens[0] = 0;
...
inline bool MessageRegistry::tick()
{
    ...
    for (size_t s = 0; s < sourcesLen; s++) {
        if (sources[s].vtable == nullptr || sources[s].vtable->tick == nullptr)
            continue;
        if (sources[s].vtable->tick(sources[s].ctx))
            any_changed = true;
    }
...
inline message_header_t *MessageRegistry::get(size_t idx) const
{
...
inline uint32_t MessageRegistry::supportedActions(size_t idx) const
{
    ...
    if (s >= sourcesLen || sources[s].vtable == nullptr)
        return 0;
    if (sources[s].vtable->supported_actions == nullptr)
        return 0;

    return sources[s].vtable->supported_actions(
        snapshots[snapshotActive][idx].hdr);
}

inline int MessageRegistry::invokeAction(size_t idx, message_action_t action)
{
    if (idx >= snapshotLens[snapshotActive])
        return -ENOENT;
    ...
    message_header_t *hdr = snapshots[snapshotActive][idx].hdr;
    ...
    int rc = sources[s].vtable->invoke_action(hdr, action);
    if (rc == 0)
        markDirty();
    return rc;
}
...
inline uint8_t MessageRegistry::sourceMode(size_t idx) const
{
    ...
    if (s >= sourcesLen || sources[s].vtable == nullptr)
        return 0;
    return sources[s].vtable->mode_id;
}
                                 ...
                                 size_t body_len, const char *recipient)
{
    ...
    for (size_t i = 0; i < sourcesLen; i++) {
        ...
        if (sources[i].vtable->send != nullptr
            && sources[i].vtable->mode_id == mode) {
            int rc = sources[i].vtable->send(sources[i].ctx, body, body_len,
                                             recipient);
            if (rc == 0)
                markDirty();
            return rc;
