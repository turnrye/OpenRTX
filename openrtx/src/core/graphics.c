            ...
            for (xx = 0; xx < w; xx++) {
                ...
                if (bits & 0x80) {
                    if (start.y + yo + yy < CONFIG_SCREEN_HEIGHT &&
                        ...
                        start.y + yo + yy > 0 && start.x + xo + xx > 0) {
                        point_t pos;
                        pos.x = start.x + xo + xx;
                        pos.y = start.y + yo + yy;
                        gfx_setPixel(pos, color);
                  ...
                  const int16_t *data, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        pos.y =
            start.y + (height / 2) + ((data[i] * 4) / (2 * SHRT_MAX) * height);
