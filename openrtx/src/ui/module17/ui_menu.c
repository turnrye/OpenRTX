...
const char *mic_gain_values[] = { "40 dB", "50 dB", "60 dB" };

const char *phase_values[] = { "Normal", "Inverted" };

const char *hwVersions[] = { "0.1d", "0.1e", "1.0" };

const char *hmiVersions[] = { "None", "1.0" };

const char *bbTuningPot[] = { "Soft", "Hard" };
    ...
    switch (index) {
        ...
        case M_CAN_RX:
            snprintf(buf, max_len, "%s",
                     (last_state.settings.m17_can_rx) ? "on" : "off");
            break;
    }
...
int _ui_getModule17ValueName(char *buf, uint8_t max_len, uint8_t index)
{
    ...
    switch (index) {
        case D_MICGAIN:
            snprintf(buf, max_len, "%s",
                     mic_gain_values[mod17CalData.mic_gain]);
            break;
        case D_PTTINLEVEL:
            snprintf(buf, max_len, "%s",
                     mod17CalData.ptt_in_level ? "Act high" : "Act low");
            break;
        case D_PTTOUTLEVEL:
            snprintf(buf, max_len, "%s",
                     mod17CalData.ptt_out_level ? "Act high" : "Act low");
            break;
        case D_TXINVERT:
            snprintf(buf, max_len, "%s",
                     phase_values[mod17CalData.bb_tx_invert]);
            break;
        case D_RXINVERT:
            snprintf(buf, max_len, "%s",
                     phase_values[mod17CalData.bb_rx_invert]);
            break;
        case D_TXWIPER:
...
int _ui_getSettingsGPSValueName(char *buf, uint8_t max_len, uint8_t index)
{
    ...
    switch (index) {
        ...
        case G_SET_TIME:
            snprintf(buf, max_len, "%s",
                     (last_state.gps_set_time) ? "ON" : "OFF");
            break;
        case G_TIMEZONE:
    ...
    switch (index) {
        ...
        case 3: // HMI Version
            ...
            if (hwinfo->flags & MOD17_FLAGS_HMI_PRESENT)
                snprintf(buf, max_len, "%s",
                         hmiVersions[(hwinfo->hw_version >> 8) & 0xFF]);
            else
...
void _ui_drawMenuGPS()
{
    ...
    point_t bar_pos = { layout.line3_pos.x + CONFIG_SCREEN_WIDTH * 1 / 3,
                        CONFIG_SCREEN_HEIGHT / 2 };
