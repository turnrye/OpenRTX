        ...
        switch(state.ui_screen)
        {
                ...
                else if(msg.keys & KEY_ENTER)
                {
                    if(idx < count)
                    {
                        ...
                        if(msg != NULL)
                        {
                            ...
                            // list may reorder or shrink while this
                            // message is open.
                            ui_state.messages_detail_seq = msg->sequence;
            ...
            case MESSAGES_DETAIL:
            {
                ...
                else if(msg.keys & KEY_ENTER)
                {
                     ...
                     * replied to. */
                    const struct message *msg = messages_get(idx);
                    if((msg != NULL) && (msg->mode == last_state.channel.mode)
                       && (msg->direction == MSG_DIR_RX))
                    {
                        /* Pre-fill compose recipient from reply sender. */
                        _ui_startCompose(msg->sender, true);
                    }
                }
#endif
                break;
                    ...
                    else if(msg.keys & KEY_ENTER)
                    {
                        ...
                        else if(ui_state.compose_focus == 2)
                        {
                            ...
                            if(clen > 0)
                            {
                                ...
                                msg.body     = ui_state.compose_body;
                                msg.bodyLen = clen;
                                msg.mode     = last_state.channel.mode;
                                strncpy(msg.sender,
                                        last_state.settings.callsign,
                                        sizeof(msg.sender) - 1);
                                strncpy(msg.recipient, rcpt,
                                        sizeof(msg.recipient) - 1);
