...
static const uint32_t zoneExtBaseAddr =
    0x31000; /**< Base address of zone extensions                       */
static const uint32_t chDataBaseAddr =
    0x110000; /**< Base address of channel data                          */
static const uint32_t contactBaseAddr =
    0x140000; /**< Base address of contacts                              */
static const uint32_t maxNumChannels =
    3000; /**< Maximum number of channels in memory                  */
static const uint32_t maxNumZones =
    250; /**< Maximum number of zones and zone extensions in memory */
...
#pragma GCC diagnostic ignored "-Waddress-of-packed-member"
    if (wcslen((wchar_t *)chData.name) == 0)
        return -1;
...
int cps_readBankHeader(bankHdr_t *b_header, uint16_t pos)
{
    ...
    W25Qx_readData(zoneExtAddr, ((uint8_t *)&zoneExtData),
                   sizeof(mduv3x0ZoneExt_t));
...
int cps_readBankData(uint16_t bank_pos, uint16_t ch_pos)
{
    ...
    uint32_t zoneExtAddr =
        zoneExtBaseAddr + bank_pos * sizeof(mduv3x0ZoneExt_t);
    W25Qx_readData(zoneAddr, ((uint8_t *)&zoneData), sizeof(mduv3x0Zone_t));
    W25Qx_readData(zoneExtAddr, ((uint8_t *)&zoneExtData),
                   sizeof(mduv3x0ZoneExt_t));
