...
static const uint16_t ctcssToneTable[] = {
    670,  719,  744,  770,  797,  825,  854,  885,  915,  948,  974,
    1000, 1035, 1072, 1109, 1148, 1188, 1230, 1273, 1318, 1365, 1413,
    1462, 1514, 1567, 1622, 1679, 1738, 1799, 1862, 1928, 2035, 2107,
    2181, 2257, 2336, 2418, 2503, 693,  625,  1598, 1655, 1713, 1773,
    1835, 1899, 1966, 1995, 2065, 2291, 2541
};
...
void HR_C6000::setRxCtcss(const tone_t tone)
{
    uint8_t index = getToneIndex(tone);
    writeCfgRegister(0xA1, 0x08); // Enable CTCSS
    writeCfgRegister(0xA7,
                     0x10); // CTCSS detection threshold, value from datasheet
...
void HR_C6000::sendTone(const uint32_t freq, const uint8_t deviation)
{
    ...
    writeCfgRegister(
        0xAF,
        0x11); // Set the same code to be sent six times (2 codes per register)
    writeCfgRegister(0xAE, 0x11);
    writeCfgRegister(0xAD, 0x11);
    writeCfgRegister(0x60, 0x00); // Disable FM transmission
    writeCfgRegister(0x60,
                     0x80); // Enable FM transmission, start sending the tone
}
