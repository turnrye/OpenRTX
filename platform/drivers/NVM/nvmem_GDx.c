...
W25Qx_DEVICE_DEFINE(eflash, eflashCfg, 0x100000) // 1 MB,  8 Mbit
    ...
    static const struct nvmDescriptor nvmDevices[] = {
        { .name = "External flash",
          ...
          .partNum = 0,
          .partitions = NULL },
        { .name = "EEPROM", .dev = &eeprom, .partNum = 0, .partitions = NULL }
    };
...
void nvm_readCalibData(void *buf)
{
    ...
    _loadBandCalData(VHF_CAL_BASE,
                     &(calib->data[0])); /* Load VHF band calibration data */
    _loadBandCalData(UHF_CAL_BASE,
                     &(calib->data[1])); /* Load UHF band calibration data */
