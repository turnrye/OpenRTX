    ...
    if (locked) {
        ...
        if (newData) {
            ...
            if (status->lsfOk) {
                ...
                // Handle incoming packet frame — content-agnostic
                else if (type == FrameType::PACKET && canMatch) {
                    PacketFrame pf = decoder.getPacketFrame();

                    if (!pktStarted) {
                        // Try to fetch the next pending RX descriptor
                    ...
                    if (pktStarted) {
                        ...
                        if (result == DeframerResult::COMPLETE) {
                            if (currRxPkt != nullptr) {
                                size_t len = pktDeframer.length();
                                if (len <= currRxPkt->size) {
                                    memcpy(currRxPkt->buffer,
                                           pktDeframer.data(), len);
                                    currRxPkt->res = (ssize_t)len;
                                    currRxPkt->status = PKT_STATUS_DONE;
                                } else {
                                    currRxPkt->res = -EMSGSIZE;
...
void OpMode_M17::txPacketState(rtxStatus_t *const status)
{
    ...
    // Initialise framer with caller-supplied data
    if (!pktFramer.init(static_cast<uint8_t *>(currTxPkt->buffer),
                        currTxPkt->size)) {
        pthread_mutex_lock(&txMutex);
        currTxPkt->res = -EINVAL;
        currTxPkt->status = PKT_STATUS_ERROR;
        currTxPkt = nullptr;
        pthread_mutex_unlock(&txMutex);
        startRx = true;
        status->opStatus = OFF;
    ...
    // Mark TX complete
    pthread_mutex_lock(&txMutex);
    currTxPkt->res = (ssize_t)currTxPkt->size;
    currTxPkt->status = PKT_STATUS_DONE;
    currTxPkt = nullptr;
    pthread_mutex_unlock(&txMutex);
