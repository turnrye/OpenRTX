...
static const uint8_t initSeq1[] = { 0x04, 0x00, 0xFF, 0xB0, 0x00, 0x00,
                                    0x00, 0x00, 0x21, 0x0B, 0xB8, 0x00 };

static const uint8_t initSeq2[] = {
    0x04, 0x0D, 0x10, 0x8C, 0xBA, 0x6C, 0x80, 0x98, 0x00, 0x00, 0x00,
    0x00, 0x33, 0xEF, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x10, 0x00,
    0x03, 0x06, 0x3B, 0xF8, 0x0E, 0x7D, 0x40, 0x7D, 0x40, 0x0B, 0x0B,
    0x17, 0x05, 0x04, 0x0B, 0x00, 0x17, 0x02, 0xFF, 0xF0, 0x28, 0x12,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x10
};

static const uint8_t initSeq3[] = { 0x04, 0xE0, 0x8B, 0x0F,
                                    0x06, 0x52, 0x27, 0x1A };

static const uint8_t initSeq4[] = { 0x01, 0x10, 0x69, 0x69, 0x96, 0x96, 0x96,
                                    0x99, 0x99, 0x99, 0xA5, 0xA5, 0xAA, 0xAA,
                                    0xCC, 0xCC, 0x00, 0xF0, 0x01, 0xFF, 0x01,
                                    0x0F, 0x00, 0x00, 0x00, 0x00, 0x0D, 0x70,
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

static const uint8_t initSeq5[] = { 0x01, 0x30, 0x00, 0x00, 0x20, 0x3C,
                                    0xFF, 0xFF, 0x3F, 0x50, 0x07, 0x60,
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

static const uint8_t initSeq6[] = { 0x01, 0x40, 0x00, 0x01, 0x01,
                                    0x02, 0x01, 0x1E, 0xF0 };

static const uint8_t initSeq7[] = { 0x01, 0x51, 0x00, 0x08, 0xEB, 0x78, 0x67 };

static const uint8_t initSeq8[] = {
    0x01, 0x60, 0x10, 0xEF, 0x00, 0x11, 0xEF, 0x00, 0x12, 0xEF, 0x00,
...
template <class M> void HR_Cx000<M>::init()
{
    ...
    writeReg(M::CONFIG, 0x06,
             0x21); // Use V_SPI for vocoder, MCU controls the vocoder
    delayMs(10);
...
template <class M> void HR_Cx000<M>::fmMode()
{
    ...
    writeReg(
        M::CONFIG, 0x36,
        0x12); // Receiving and opening the voice channel in FM mode Codec switch, 1 means on, 0 means off.
    writeReg(M::CONFIG, 0x20, 0x00); // Local access policy, important
    writeReg(
        M::CONFIG, 0x21,
        0x01); // Control enable to clear the data in the vocoder decoding cache buffer
    writeReg(
        M::CONFIG, 0x21,
        0x02); // Control enable for clearing the data in the vocoder encoding buffer
    ...
    writeReg(
        M::CONFIG, 0x41,
        0x20); // SyncFail = 1, no synchronization information exists, requiring the physical layer to search again.
    writeReg(M::CONFIG, 0x41, 0x00); // SyncFail = 0
    writeReg(
        M::CONFIG, 0x41,
        0x40); // RxNxtSlotEn = 1, Start receiving the interrupt for the upcoming time slot. receive
    writeReg(M::CONFIG, 0x40, 0x43); // RxEn = 1, receive synchronization active
    writeReg(
        M::CONFIG, 0xE0,
        0x8B); // CPU controls the codec, LineOut2 enabled, Mic_p enabled, HR_C6000 is I2S slave
    ...
    writeReg(M::CONFIG, 0x3E,
             0x06); // FM receiving end modulation frequency offset coefficient
    writeReg(M::CONFIG, 0x81, 0x00); // InterClass1Mask, all interrupts masked
    writeReg(M::CONFIG, 0x60, 0x00); // TransControl, all off
    writeReg(
        M::CONFIG, 0x34,
        0xBC); // Band pass filter on, pre-emphasis on, wide bandwidth, 25kHz bandiwdth, RX bandwidth 25kHz
    ...
    writeReg(
        M::CONFIG, 0xE0,
        0x83); // CPU controls the codec, Mic_p enabled, HR_C6000 is I2S slave
    writeReg(M::CONFIG, 0x36, 0x10); // Vocoder codec packet interface enabled
    writeReg(
        M::CONFIG, 0x36,
        0x12); // Receiving and opening the voice channel in FM mode Codec switch, 1 means on, 0 means off.
    writeReg(M::CONFIG, 0xE0,
             0xC9); // Codec enabled, LineIn1, LineOut2, I2S slave mode
    writeReg(M::CONFIG, 0x26,
             0xFE); // Undocumented register, disable FM audio output
}
...
void HR_Cx000<M>::startAnalogTx(const TxAudioSource source, const FmConfig cfg)
{
    ...
    if (source == TxAudioSource::MIC)
        audioCfg |= 0x02;
    if (source == TxAudioSource::LINE_IN)
        audioCfg |= 0x40;
