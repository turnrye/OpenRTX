...
int main()
{
    ...
    char *portname =
        "/dev/serial/by-id/usb-STMicroelectronics_STM32_Virtual_ComPort_in_FS_Mode_00000000050C-if00";
    ...
    while (true) {
        ...
        fprintf(csv_log, "%" PRId16 ",%d,%f,%d,%f,%f,%d,%d,%d\n", log.sample,
                log.conv, log.conv_th, log.sample_index, log.qnt_pos_avg,
                log.qnt_neg_avg, log.symbol, log.frame_index, log.flags);
        fflush(csv_log);
    }
    fclose(csv_log);
}
