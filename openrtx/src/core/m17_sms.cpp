...
/**
  ...
  *     * On TX completion posts a pkt_tx_done_t (status = SENT or FAILED).
  *     * On RX completion parses payload; posts pkt_rx_event_t (received text).
  *
 ...
 * embedded targets.  pkt_tx_request_t (~843 B) cannot be allocated as
 * a local variable in that context. */
static pkt_tx_request_t rtx_req;
static pkt_tx_done_t rtx_tx_done; /* TX completion retry buffer */
...
/**
 * UI-thread tick: drain pkt_rx_event_t and pkt_tx_done_t from packet_io.
 *
...
static bool sms_tick(void *ctx)
{
    ...
    static pkt_rx_event_t rx_evt;
    static pkt_tx_done_t tx_done;
        ...
        for (size_t i = 0; i < M17_SMS_MAX_MESSAGES; i++) {
            if (entries[i].active && entries[i].tag == tx_done.tag
                ...
                entries[i].hdr.status =
                    static_cast<message_status_t>(tx_done.status);
                changed = true;
                break;
            }
        }
