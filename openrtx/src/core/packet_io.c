 ...
 * into all builds but the static queue slots and queue functions are only
 * instantiated when CONFIG_M17_SMS is defined (i.e. when at least one
 * packet-mode message source is present).  Targets with no message sources
 * (e.g. Module17) get only an empty packet_io_init() stub, saving ~1.7 KB
 * of BSS.
 */
