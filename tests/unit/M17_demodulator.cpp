...
int main()
{
    // Open file
    FILE *baseband_file =
        fopen("../tests/unit/assets/M17_test_baseband_dc.raw", "rb");
    ...
    size_t read_items =
        fread(baseband_buffer, 2, baseband_samples, baseband_file);
    ...
    for (unsigned i = 0;
         i < baseband_samples - m17Demodulator.M17_SYNCWORD_SYMBOLS *
    ...
    do {
        ...
        offset = syncword.index + m17Demodulator.M17_SYNCWORD_SYMBOLS *
                                      m17Demodulator.M17_SAMPLES_PER_SYMBOL;
        printf("%d\n", syncword.index);
    ...
    for (unsigned i = 0;
        ...
        if ((int)i == (syncword.index + 1)) {
            ...
            syncword = m17Demodulator.nextFrameSync(
                syncword.index + m17Demodulator.M17_SYNCWORD_SYMBOLS *
                                     m17Demodulator.M17_SAMPLES_PER_SYMBOL);
    ...
    FILE *symbols_ref =
        fopen("../tests/unit/assets/M17_test_baseband.bin", "rb");
    // Skip preamble
