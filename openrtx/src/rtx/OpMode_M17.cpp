            ...
            if (status->lsfOk) {
                ...
                // Handle incoming packet frame; CAN and callsign filters apply
                    ...
                    if (currRxPkt == nullptr) {
                        ...
                        if (currRxPkt != nullptr)
                            pktDeframer.init(
                                static_cast<uint8_t *>(currRxPkt->buffer),
                                currRxPkt->size);
                        else
                    ...
                    if (result == DeframerResult::COMPLETE) {
                        if (currRxPkt != nullptr) {
                            // Zero-copy: data already in currRxPkt->buffer
...
void OpMode_M17::txPacketState(rtxStatus_t *const status)
{
    ...
    if (!pktTxStarted) {
        if (!pktFramer.init(static_cast<uint8_t *>(pkt->buffer), pkt->size)) {
            pkt->res = -EINVAL;
        ...
        /* Prefer per-packet destination if set, else global config. */
        const char *dst_addr = (pkt->destination[0] != '\0') ?
                                   pkt->destination :
                                   status->destination_address;
        Callsign dst(dst_addr);
        if (!dst.isEmpty())
            lsf.setDestination(dst);
