...
private:
    ...
    NbRingBuffer<struct pktDesc *, 4> rxPktQueue; ///< Pending RX descriptors
    struct pktDesc *currRxPkt;                    ///< Descriptor being filled

    struct pktDesc *currTxPkt; ///< Pending TX descriptor (single-slot)
    pthread_mutex_t txMutex;   ///< Protects currTxPkt
};
