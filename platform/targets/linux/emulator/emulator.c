...
static keyboard_t keyname2keyboard(char *name)
{
    ...
    char *names[] = {
        ...
        "KEY_VOLUP", "KEY_VOLDOWN", "KNOB_LEFT", "KNOB_RIGHT",
    };
...
static int toggleVariable(void *_self, int _argc, char **_argv)
{
    ...
    _climenu_option *self = (_climenu_option *)_self;
    *(int *)self->var =
        !*(int *)self->var; //yeah, maybe this got a little out of hand
...
static _climenu_option _options[] = {
    ...
    { "key",
      "Press keys in sequence (e.g. 'key ENTER DOWN ENTER' will descend through two menus)",
      NULL, pressKey },
    { "keycombo", "Press a bunch of keys simultaneously", NULL,
      pressMultiKeys },
    { "show", "Show current radio state (ptt, rssi, etc)", NULL, printState },
    { "screenshot",
      "[screenshot.bmp] Save screenshot to first arg or screenshot.bmp if none given",
      NULL, screenshot },
