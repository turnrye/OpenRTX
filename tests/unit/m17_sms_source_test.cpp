          ...
          "[m17_sms]")
{
    ...
    for (int i = 0; i < M17_SMS_MAX_MESSAGES * 2 + 5; i++) {
        ...
        m17_sms_vtable.send(NULL, body, (size_t)n, "N0CALL");
        packet_io_init(); /* clear single-slot TX queue for the next send */

        /* Independently count reachable entries and compare to count(). */
        size_t scanned = 0;
        while (m17_sms_vtable.get(NULL, scanned) != nullptr)
            scanned++;
        REQUIRE(m17_sms_vtable.count(NULL) == scanned);
    }

    /* At steady state the inbox is full, not unbounded. */
    REQUIRE(m17_sms_vtable.count(NULL) == (size_t)M17_SMS_MAX_MESSAGES);

    /* Deleting entries keeps the counter exactly in step. */
    while (m17_sms_vtable.count(NULL) > 0) {
        size_t before = m17_sms_vtable.count(NULL);
        message_header_t *h = m17_sms_vtable.get(NULL, 0);
        REQUIRE(h != nullptr);
        m17_sms_vtable.invoke_action(h, MSG_ACTION_DELETE);
        REQUIRE(m17_sms_vtable.count(NULL) == before - 1);
    }
    REQUIRE(m17_sms_vtable.count(NULL) == 0);
}
